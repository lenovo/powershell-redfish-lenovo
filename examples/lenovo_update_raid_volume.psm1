...
function lenovo_update_raid_volume
{
    <#
   .Synopsis
    Cmdlet used to update RAID volume
   .DESCRIPTION
    Cmdlet used to update RAID volume from BMC using Redfish API. Set result will be printed to the screen. Connection information can be specified via command parameter or configuration file.
   ...
   .EXAMPLE
    lenovo_update_raid_volume -ip 10.10.10.10 -username admin -password admin -raidid RAID_Slot1 -volume_name test_volume1 -drive_cache_policy Disable
   #>
    ...
    try
    {
        ...
        $response = Invoke-WebRequest -Uri $volume_url -Method Patch -Headers $JsonHeader -Body $JsonBody -ContentType 'application/json'
        Write-Host
        [String]::Format("- PASS, statuscode {0} returned successfully to update volume {1}",$response.StatusCode,$volume_name)
