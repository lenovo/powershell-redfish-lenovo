...
function lenovo_httpfs_certificate_enable
{
   <#
   .Synopsis
    Cmdlet used to enable https certificate
   .DESCRIPTION
    Cmdlet used to enable https certificate from BMC using Redfish API. Information will be printed to the screen. Connection information can be specified via command parameter or configuration file.
    - ip: Pass in BMC IP address
    - username: Pass in BMC username
    - password: Pass in BMC username password
    - config_file: Pass in configuration file path, default configuration file is config.ini
   .EXAMPLE
    lenovo_httpfs_certificate_enable -ip 10.10.10.10 -username USERID -password PASSW0RD
   #>
    ...
    try 
    {
        ...
        {
            if ($hash_table.VerifyRemoteServerCertificate -eq $True) 
            {
                Write-Host "HTTPS certificate security is already enabled."
                return $True
            }
            $enable_body = @{"VerifyRemoteServerCertificate" = $True} | ConvertTo-Json
            $response_enable_verify = Invoke-WebRequest -Uri $update_service_url -Method Patch -Headers $JsonHeader -Body $enable_body -ContentType 'application/json' 
            if ($response_enable_verify.statuscode -eq 200) 
            {
                Write-Host "HTTPS certificate security is enabled."
