...
function lenovo_ldap_certificate_enable
{
    <#
   .Synopsis
    Cmdlet used to enable LDAP certificate
   .DESCRIPTION
    Cmdlet used to enable LDAP certificate from BMC using Redfish API. Information will be printed to the screen. Connection information can be specified via command parameter or configuration file.
   ...
   .EXAMPLE
    lenovo_ldap_certificate_enable -ip 10.10.10.10 -username USERID -password PASSW0RD
   #>
    ...
    try
    {
        ...
        foreach ($i in $managers_hash_table.Members)
        {
            ...
            if ($enable_ldap -eq $True)
            {
                return "LDAP certificate security is already enabled."
            }

            # Create request body
            $Body = @{}
            $Body['SSLSettings'] = @{}
            $Body['SSLSettings']['EnableLDAPS'] = $True
            ...
            [String]::Format("- PASS, statuscode {0} returned successfully to set ldap certificate enable. Note that in order to enable SSL, a valid SSL certificate must first be in place and at least one SSL client trusted certificate must be imported.",$response.StatusCode)
            return 
        }
                ...
                if ($null -ne $account_service_hash_table.LDAP.'Authentication'.'Oem')
                {
                    ...
                    {        
                        if ($null -ne $account_service_hash_table.LDAP.'Authentication'.'Oem'.'Ami'.'EncryptionType')
                        {
                            ...
                            if ($encryption_type -eq 'SSL')
                            {
                                return "LDAP certificate security is already enabled."
                            }
                            elseif ($encryption_type -eq 'StartTLS')
                            {
                                return "LDAP certificate security is already enabled, EncryptionType is StartTLS. Note: All of the 3 files(CA certificate/Client certificate/Client private key) are required for StartTLS. Because script can only support import CA certificate, please use webUI to import certificates instead of using script."
                            }
                            elseif ($encryption_type == 'NoEncryption')
                            {
                                if ($null -eq $binddn -or $null -eq $bindpassword)
                                {
                                    return "Parameter binddn and bindpassword are needed for enabling LDAP certificate security."
                                }
                                ...
                                $ldap_body['LDAP']['Authentication']['Oem']['Ami']['EncryptionType'] = "SSL"
                                ...
                                [String]::Format("- PASS, statuscode {0} returned successfully to set ldap certificate enable. Note that in order to enable SSL, CA certificate that contains the certificate of the trusted CA certs must be imported before using.",$response.StatusCode)
                                return 
                            }
                        }
                    }
