###
#
# Lenovo Redfish examples - Get serial interfaces
#
# Copyright Notice:
#
...
function get_serial_interfaces
{
   <#
   .Synopsis
    Cmdlet used to get serial interfaces
   .DESCRIPTION
    Cmdlet used to get serial interfaces using Redfish API
    Connection information can be specified via command parameter or configuration file
    - ip: Pass in BMC IP address
    - username: Pass in BMC username
    - password: Pass in BMC username password
    - interfaceid: Pass in serial interface instance id
    - config_file: Pass in configuration file path, default configuration file is config.ini
   .EXAMPLE
    get_serial_interfaces -ip 10.10.10.10 -username USERID -password PASSW0RD -interfaceid 1
   #>
    ...
    (
        ...
        [Parameter(Mandatory=$False)]
        [string]$password="",
        [Parameter(Mandatory=$False, HelpMessage='Serial interface instance id(default first instance)')]
        [int]$interfaceid= 1,
        [Parameter(Mandatory=$False)]
    ...
    try
    {
        ...
        foreach ($manager_url_string in $manager_url_collection)
        {
            ...
            # Get the manager url from the manager url collection
            ...
            $serial_interfaces_url_collection = @()
            foreach ($i in $hash_table.Members)
            {
                $i = [string]$i
                $serial_interfaces_url = ($i.Split("=")[1].Replace("}",""))
                $serial_interfaces_url_collection += $serial_interfaces_url
            }

            $count = $serial_interfaces_url_collection.Count-1
            if(($interfaceid-1 -lt 0) -or ($interfaceid-1 -gt $count))
            {
            ...
            $serial_interfaces_x_url = "https://$ip" + $serial_interfaces_url_collection[$index]

            # Get the serial interfaces information
