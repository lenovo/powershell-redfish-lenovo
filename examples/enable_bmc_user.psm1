###
#
# Lenovo Redfish examples - Enable BMC User
#
# Copyright Notice:
#
...
function enable_bmc_user
{
   <#
   .Synopsis
    Cmdlet used to enable BMC user account
   .DESCRIPTION
    Cmdlet used to enable BMC user account using Redfish API
    ...
    try
    {
        ...
        foreach ($i in $hash_table.Members)
        {
            ...
            if($hash_table.UserName -eq $userid)
            {
                ...
                Write-Host
                [String]::Format("- PASS, statuscode {0} returned successfully to enabled user {1}",$response.StatusCode,$hash_table.UserName)
